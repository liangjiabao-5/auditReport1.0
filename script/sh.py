...
def resultDeal(chatResult):
    ...
    if "结果：符合" in content:
        result = "符合"
    elif  "结果：部分符合" in content:
        result = "部分符合"
    elif "结果：不符合" in content:
        result = "不符合"
    else:
        result = None
...
if __name__ == "__main__":
    ...
    # 加载已存在的 Excel 文件
    wb = openpyxl.load_workbook("测试优化数据结果4.0.xlsx")
    ws = wb.active
...
#     realy_result_col = existing_columns.index("realy_result") + 1
    ...
    for i, data in enumerate(security_control_class_data, start=0):
        ...
        expectResult = getExpect("安全物理环境", security_control_class_data[i], quota)
